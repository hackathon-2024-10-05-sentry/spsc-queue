...
pub fn SPSCQueue(comptime T: type) type {
    ...
    return struct {
        ...
        pub fn init(self: *Self, allocator: *std.mem.Allocator, requested_capacity: usize) !void {
            ...
            adjusted_capacity += 1; // Add a slack element to differentiate full and empty states

            // Calculate the maximum capacity to prevent usize overflow during allocation
            // Explanation:
            // - std.math.maxInt(usize) returns the maximum value that a usize can hold
            // - We subtract (2 * kPadding) because we will later add this padding to adjusted_capacity
            //   when calculating total_slots (adjusted_capacity + 2 * kPadding)
            // - This ensures that total_slots does not exceed the maximum usize value,
            //   preventing overflow when allocating memory
            const max_capacity = std.math.maxInt(usize) - 2 * kPadding;

            // If the adjusted capacity exceeds the maximum allowable capacity, clamp it
            ...
            // Allocate slots with additional padding to prevent false sharing between producer and consumer
